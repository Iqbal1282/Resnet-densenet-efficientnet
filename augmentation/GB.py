...
def gaussian_kernel(kernel_size, var_x, var_y):
    ...
    mdvice = 'cuda' if torch.cuda.is_available() else 'cpu'
    x = ax.view(1, -1).repeat(ax.size(0), 1).to(mdvice)
    y = ax.view(-1, 1).repeat(1, ax.size(0)).to(mdvice)
    x2 = torch.pow(x,2)
    y2 = torch.pow(y,2)
...
class GB2d(nn.Module):
    def __init__(self, in_channel, kernel_size, var_x = None, var_y = None, aug_train = True, padding = 0):
        ...
        if self.aug_train:
            ...
            mdvice = 'cuda' if torch.cuda.is_available() else 'cpu'
            self.var_x = torch.FloatTensor([var_x]).to(mdvice)
            self.var_y = torch.FloatTensor([var_y]).to(mdvice)
